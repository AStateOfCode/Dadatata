...
class Image extends BaseReader {
    ...
    public function extract($path)
    {
        ...
        if(($match = $this->match('/\d/', $track->Width)) !== null) $result[ReaderInterface::IMAGE_WIDTH] = $match;
        if(($match = $this->match('/\d/', $track->Height)) !== null) $result[ReaderInterface::IMAGE_HEIGHT] = $match;
        if(($match = $this->match('/\d/', $track->Bit_depth)) !== null) $result[ReaderInterface::IMAGE_BIT_DEPTH] = $match;
        if(isset($track->Color_space)) $result[ReaderInterface::IMAGE_COLORSPACE] = (string)$track->Color_space;
