...
class ChainFilterTest extends BaseTestCase
{
    ...
    public function testProcessWithOneChildFilter()
    {
        ...
        $filter1->expects($this->once())->method('process')->with($thing, $path, $options)->will(
    ...
    public function testProcessWithMultipleChildFilter()
    {
        ...
        $filter1 = $this->createEmptyFilterMock();
        $filter1->expects($this->once())->method('process')->with($thing, $path, $options)->will(
            $this->returnValue($expectedResult1)
        );
        $filter2 = $this->createEmptyFilterMock();
        $filter2->expects($this->once())->method('process')->with($thing, $expectedResult1[0], $options)->will(
            $this->returnValue($expectedResult2)
        );
        $filter3 = $this->createEmptyFilterMock();
        $filter3->expects($this->once())->method('process')->with($thing, $expectedResult2[0], $options)->will(
            $this->returnValue($expectedResult3)
        );
